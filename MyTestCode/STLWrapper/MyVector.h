...
public:
	...
	//O( N Log N ) : 내림차순 정렬
	void SortAll_Reverse() {
		std::sort(super::rbegin(), super::rend());
	}

	//O( log N ) : 오름차순 정렬 상태에서 사용 가능
	bool Binary_search(const _Ty& val) {
		return std::binary_search(super::begin(), super::end(), val);
	}
	//O( log N ) : 내림차순 정렬 상태에서 사용 가능
	bool Binary_search_Reverse(const _Ty& val) {
		return std::binary_search(super::rbegin(), super::rend(), val);
	}
